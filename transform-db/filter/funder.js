...
const f = async () => {
  try {
    ...
    // select funder_id, json_arrayagg(json_object('funder_name', funder_name, 'last_date', last_date)) from 
    // (select funder_id, funder_name, max(date_awarded) as last_date from grantnav_grant where funder_name is not null group by funder_id, funder_name) as t1 
    // group by funder_id;
    const query = knex
      .select([
